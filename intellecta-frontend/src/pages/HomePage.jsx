    ...
    >
      ...
      <AnimatePresence>
        {!scrolled && (
          ...
          >
            {links.map(l => (
              <a key={l} href={`#${l.toLowerCase().replace(/\s+/g, "-")}`} className="nav-link-top">{l}</a>
            ))}
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "4rem 2rem 5rem", width: "100%", display: "grid", gridTemplateColumns: "1.05fr 0.95fr", gap: "3.5rem", alignItems: "center" }}>
        <motion.div initial="hidden" animate="visible">
          ...
          <motion.h1 variants={fadeUp} custom={1}
            style={{ fontSize: "clamp(2.8rem,5.8vw,5rem)", lineHeight: 1.08, fontWeight: 400, letterSpacing: "-0.03em", marginBottom: "1.4rem" }}>
            <span className="serif" style={{ color: "var(--ink)", display: "block" }}>Focus Deeper.</span>
            <span className="serif cyan-text" style={{ fontStyle: "italic", display: "block" }}>Learn Smarter.</span>
        ...
        <motion.div style={{ y: floatY, display: "flex", flexDirection: "column", gap: "0.9rem" }} className="hero-right">
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }}
            ...
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.09em", textTransform: "uppercase", opacity: 0.65, marginBottom: 6 }}>CURRENT STANDING</div>
            ...
            <div style={{ fontSize: 10, opacity: 0.6, marginBottom: 5, display: "flex", justifyContent: "space-between" }}>
            ...
            <div style={{ marginTop: 12, fontSize: 11, opacity: 0.6 }}>550 XP to next level · Deep Thinker badge</div>
          </motion.div>
  ...
  return (
    <div style={{ background: "linear-gradient(135deg,#1DA8B8 0%,#53D2E0 100%)", padding: "13px 0", overflow: "hidden", boxShadow: "0 4px 20px rgba(83,210,224,0.28)" }}>
      <div style={{ display: "flex", animation: "ticker 28s linear infinite", width: "max-content" }}>
        {doubled.map((item, i) => (
          <span key={i} className="syne" style={{ fontSize: 11, fontWeight: 700, whiteSpace: "nowrap", padding: "0 1.8rem", color: "rgba(255,255,255,0.9)", letterSpacing: "0.06em", textTransform: "uppercase" }}>
...
const FEATURES = [
  ...
  { icon: BookOpen,     color: "#06B6D4", label: "Coverage Tracker",title: "Mastery, Topic by Topic",     desc: "Granular per-topic status — Mastered, In Progress, Reviewed, Not Started — paired with a Panic Meter and smart daily study target." },
  ...
  return (
    <motion.div ref={ref}
      ...
      style={{
        ...
        border: `1.5px solid ${hov ? feat.color + "66" : "rgba(255,255,255,0.6)"}`,
        borderRadius: 18, padding: "1.8rem",
        boxShadow: hov ? `0 10px 40px ${feat.color}22` : "0 2px 14px rgba(83,210,224,0.06)",
        transform: hov ? "translateY(-5px)" : "translateY(0)",
        transition: "all 0.28s ease", cursor: "default",
      }}>
      <div style={{ width: 48, height: 48, borderRadius: 14, background: `${feat.color}16`, border: `1.5px solid ${feat.color}30`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "1.2rem", boxShadow: hov ? `0 4px 18px ${feat.color}28` : "none", transition: "box-shadow 0.28s" }}>
        <feat.icon size={22} color={feat.color} />
      </div>
      <div style={{ fontSize: 10, color: feat.color, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 8 }}>{feat.label}</div>
      <h3 className="syne" style={{ fontSize: "0.97rem", fontWeight: 700, color: "var(--ink)", marginBottom: "0.6rem", lineHeight: 1.3 }}>{feat.title}</h3>
      <p style={{ fontSize: 13.5, color: "var(--ink-light)", lineHeight: 1.72 }}>{feat.desc}</p>
    </motion.div>
  );
}
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <motion.div variants={fadeUp} custom={0} style={{ textAlign: "center", marginBottom: "4rem" }}>
          ...
          <h2 className="serif" style={{ fontSize: "clamp(2.2rem,4.5vw,3.6rem)", marginBottom: "1rem", lineHeight: 1.15, color: "var(--ink)" }}>
            Built for the <span className="cyan-text" style={{ fontStyle: "italic" }}>relentlessly curious</span>
          </h2>
...
function StepCard({ step, i }) {
  ...
  return (
    <motion.div ref={ref}
      ...
      style={{ background: "rgba(255,255,255,0.72)", backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)", border: "1.5px solid rgba(255,255,255,0.6)", borderRadius: 20, padding: "2rem 1.6rem", boxShadow: "0 2px 14px rgba(83,210,224,0.07)", position: "relative" }}>
      <div className="syne" style={{ position: "absolute", top: 18, right: 20, fontSize: 11, fontWeight: 800, color: step.color, opacity: 0.35, letterSpacing: "0.04em" }}>{step.n}</div>
      ...
      <h3 className="syne" style={{ fontSize: "0.97rem", fontWeight: 700, color: "var(--ink)", marginBottom: "0.6rem" }}>{step.title}</h3>
      <p style={{ fontSize: 13.5, color: "var(--ink-light)", lineHeight: 1.72 }}>{step.desc}</p>
    </motion.div>
  );
}
...
function HowItWorks() {
  return (
    <InViewSection id="how-it-works" style={{ padding: "6rem 2rem", background: "rgba(216,212,240,0.35)", backdropFilter: "blur(8px)" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <motion.div variants={fadeUp} custom={0} style={{ textAlign: "center", marginBottom: "4.5rem" }}>
          <h2 className="serif" style={{ fontSize: "clamp(2.2rem,4.5vw,3.6rem)", color: "var(--ink)", marginBottom: "1rem" }}>
            How <span className="cyan-text" style={{ fontStyle: "italic" }}>Intellecta</span> works
          </h2>
...
function ScreenCard({ s, i }) {
  ...
  return (
    <motion.div ref={ref}
      ...
      style={{ background: "rgba(255,255,255,0.72)", backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)", borderRadius: 18, overflow: "hidden", border: "1.5px solid rgba(255,255,255,0.6)", boxShadow: "0 2px 14px rgba(83,210,224,0.06)" }}>
      <div style={{ height: 130, background: s.bg, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
        <div style={{ width: 60, height: 60, borderRadius: 17, background: `${s.color}20`, border: `2px solid ${s.color}35`, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ padding: "1.2rem" }}>
        <div style={{ fontSize: 10, color: s.color, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 5 }}>{s.label}</div>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <motion.div variants={fadeUp} custom={0} style={{ textAlign: "center", marginBottom: "3.5rem" }}>
          <h2 className="serif" style={{ fontSize: "clamp(2rem,4vw,3.2rem)", color: "var(--ink)", marginBottom: "0.8rem" }}>
            Every page. <span className="cyan-text" style={{ fontStyle: "italic" }}>One purpose.</span>
          </h2>
...
function AppShowcase() {
  ...
  return (
    <InViewSection style={{ padding: "4rem 2rem", background: "rgba(255,255,255,0.4)" }}>
      <div style={{ maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
        <h2 className="serif" style={{ fontSize: "clamp(2rem,4vw,3.2rem)", color: "var(--ink)", marginBottom: "2.5rem" }}>
          See <span className="cyan-text" style={{ fontStyle: "italic" }}>Intellecta</span> in Action
        </h2>
        
        <div style={{ position: "relative", width: "100%", aspectRatio: "16/9", borderRadius: 24, overflow: "hidden", boxShadow: "0 20px 60px rgba(83,210,224,0.18)", border: "1px solid rgba(255,255,255,0.7)", background: "rgba(255,255,255,0.6)" }}>
          ...
          {/* Controls */}
          <button onClick={prev} style={{ position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)", width: 48, height: 48, borderRadius: "50%", background: "rgba(255,255,255,0.85)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.4)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 6px 16px rgba(0,0,0,0.08)", zIndex: 10 }}>
            <ChevronLeft size={24} color="var(--hp-ink)" style={{ marginLeft: -2 }} />
          </button>
          <button onClick={next} style={{ position: "absolute", right: 16, top: "50%", transform: "translateY(-50%)", width: 48, height: 48, borderRadius: "50%", background: "rgba(255,255,255,0.85)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.4)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 6px 16px rgba(0,0,0,0.08)", zIndex: 10 }}>
            <ChevronRight size={24} color="var(--hp-ink)" style={{ marginRight: -2 }} />
          </button>

          {/* Dots */}
          <div style={{ position: "absolute", bottom: 24, left: "50%", transform: "translateX(-50%)", display: "flex", gap: 10, zIndex: 10 }}>
            {SCREENSHOTS.map((_, i) => (
              <button key={i} onClick={() => setIndex(i)} style={{ width: 12, height: 12, borderRadius: "50%", border: "1px solid rgba(255,255,255,0.5)", cursor: "pointer", background: i === index ? "var(--hp-cyan)" : "rgba(255,255,255,0.4)", transition: "all 0.3s ease", padding: 0 }} />
            ))}
          </div>
        </div>
      </div>
...
function PillarCard({ p, i }) {
  ...
  return (
    <motion.div ref={ref}
      ...
      style={{ background: "rgba(255,255,255,0.72)", backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)", border: "1.5px solid rgba(255,255,255,0.6)", borderRadius: 16, padding: "1.4rem", boxShadow: "0 2px 14px rgba(83,210,224,0.06)" }}>
      ...
      <h4 className="syne" style={{ fontSize: 13.5, fontWeight: 700, color: "var(--ink)", marginBottom: "0.4rem" }}>{p.title}</h4>
      <p style={{ fontSize: 12.5, color: "var(--ink-light)", lineHeight: 1.65 }}>{p.desc}</p>
    </motion.div>
  );
}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "5rem", alignItems: "center" }}>
          <motion.div variants={fadeUp} custom={0}>
            ...
            <h2 className="serif" style={{ fontSize: "clamp(2rem,4vw,3.2rem)", lineHeight: 1.15, marginBottom: "1.4rem", color: "var(--ink)" }}>
              ...
              <span className="cyan-text" style={{ fontStyle: "italic" }}>A cognitive system.</span>
            </h2>
  ...
  return (
    <InViewSection style={{ padding: "7rem 2rem" }}>
      <div style={{ maxWidth: 860, margin: "0 auto", textAlign: "center" }}>
        <motion.div variants={fadeUp} custom={0}
          style={{ background: "linear-gradient(145deg,#1DA8B8 0%,#53D2E0 55%,#7EEAF4 100%)", borderRadius: 28, padding: "5rem 3rem", boxShadow: "0 20px 80px rgba(83,210,224,0.38)", position: "relative", overflow: "hidden" }}>
          ...
          <motion.h2 variants={fadeUp} custom={2} className="serif"
            ...
            Your sanctuary<br /><span style={{ fontStyle: "italic", opacity: 0.85 }}>awaits.</span>
          </motion.h2>
