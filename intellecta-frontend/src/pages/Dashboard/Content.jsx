...
import { 
  Plus, Edit2, Trash2, Brain, Database, 
  Zap, BarChart3, Upload, FileText, Users 
} from 'lucide-react';
  ...
  return (
    <div className="flex min-h-screen bg-[#F9FAFB] font-inter text-[#111827]">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="p-10 max-w-[1450px] mx-auto w-full">
          <div className="flex justify-between items-start mb-10">
            <div>
              <h2 className="text-4xl font-black tracking-tight text-[#111827]">Content Repository</h2>
              <p className="text-gray-400 font-bold mt-2">Manage pedagogical structures and knowledge diagnostics.</p>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-8 space-y-8">
              {subjects.map((subject) => (
                <div key={subject.id} className="bg-white rounded-[40px] p-8 border border-gray-100 shadow-sm">
                  <div className="flex justify-between items-start mb-8">
                    <div className="flex gap-5">
                      <div className={`${subject.bgColor} p-4 rounded-2xl`}>
                        {React.isValidElement(subject.icon) ? React.cloneElement(subject.icon, { size: 28 }) : null}
                      </div>
                      <div>
                        <h3 className="text-2xl font-black">{subject.title}</h3>
                        <div className="flex items-center gap-3 mt-1">
                          <span className="text-[#6C5DD3] font-black text-xs uppercase tracking-widest">{subject.code}</span>
                          <span className="text-gray-300">•</span>
                          <span className="text-gray-400 font-bold text-xs uppercase tracking-widest">{subject.topics} Topics</span>
                          <span className="text-gray-300">•</span>
                          <span className="text-gray-400 font-bold text-xs uppercase tracking-widest">{subject.students} Students</span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {subject.subTopics.map((topic, idx) => (
                      <button key={idx} className="flex items-center gap-3 bg-gray-50 p-5 rounded-[24px] border border-gray-100 hover:border-[#6C5DD3]/30 transition-all text-left group">
                        <div className="h-1.5 w-1.5 rounded-full bg-gray-300 group-hover:bg-[#6C5DD3]" />
                        <span className="text-sm font-black text-gray-700">{topic}</span>
                      </button>
                    ))}
                  </div>
                  ...
                  <button className="w-full mt-4 border-2 border-dashed border-gray-100 rounded-[24px] py-4 flex items-center justify-center gap-2 text-indigo-400 hover:bg-indigo-50/50 transition-all group">
                    <Plus size={18} strokeWidth={3} className="group-hover:scale-110" />
                    <span className="text-xs font-black uppercase tracking-widest">Add New Topic</span>
                  </button>
                </div>
              ))}
            </div>
            ...
            <div className="lg:col-span-4 space-y-6">
              <div className="bg-[#1C1C2D] rounded-[40px] p-8 text-white shadow-2xl">
                ...
                <p className="text-xs text-gray-400 font-bold leading-relaxed mb-8">
                  System-wide AI monitoring active quiz banks to detect student knowledge gaps.
                </p>
  ...
  return (
    <div className="bg-white/5 rounded-[24px] p-5 border border-white/5">
      <div className="flex justify-between items-start mb-3">
        <span className={`${tagColor} text-[8px] font-black px-2 py-1 rounded-md uppercase tracking-wider`}>
          {tag}
        </span>
      </div>
      ...
      <div className="flex justify-between items-center">
        ...
        <button className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">{actionText}</button>
      </div>
    </div>
  );
};
...
const QuickLink = ({ icon, text }) => (
  <button className="flex items-center gap-4 w-full group text-left">
    <div className="bg-white p-2 rounded-xl border border-gray-100 group-hover:bg-indigo-50 group-hover:text-[#6C5DD3] transition-colors text-gray-400">
      {icon && React.isValidElement(icon) ? React.cloneElement(icon, { size: 18 }) : null}
    </div>
    <span className="text-sm font-bold text-gray-600 group-hover:text-[#111827]">{text}</span>
  </button>
);
