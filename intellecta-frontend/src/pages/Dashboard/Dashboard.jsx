...
const Dashboard = () => {
  ...
  const alerts = [
    {
      id: 1,
      title: "Anomalous Session Duration",
      desc: "User ID: 88219 logged 18+ consecutive focus hours.",
      time: "2 min ago",
      type: "CRITICAL",
      icon: AlertTriangle,
      color: "text-red-500",
...

    {
      ...
      desc: "EU-West node experiencing >400ms response times.",
      time: "14 min ago",
      type: "WARNING",
      icon: Tablet,
      color: "text-gray-500",
    ...
    {
      id: 3,
      title: "Account Safeguard Triggered",
      desc: "Multiple login attempts from unverified IP (Beijing, CN).",
      time: "1 hr ago",
      type: "RESOLVED",
      icon: ShieldCheck,
  ...
  return (
    <div className="flex-1 flex flex-col">
      {/* PROPS: Passing the logo to the Navbar */}
      <Navbar intellectaLogo={intellectaLogo} />

      <div className="flex min-h-screen bg-[#F9FAFB] font-inter">
        {/*PROPS: Passing state to Sidebar so it knows which button is highlighted */}
              ...
              <div className="lg:col-span-8 bg-white rounded-[40px] p-10 border border-gray-100 shadow-sm">
                ...
                <div className="relative flex items-end justify-between h-64 w-full px-2">
                  {[55, 68, 40, 92, 60, 35, 30].map((h, i) => (
                    ...
                    >
                      <div
                        className={`w-full rounded-md transition-all duration-300 ${h === 92 ? "bg-[#6C5DD3]" : "bg-[#E3E0F7]"}`}
                        style={{ height: `${h}%` }}
                      ></div>
              ...
              <div className="lg:col-span-4 bg-white rounded-[40px] p-10 border border-gray-100 shadow-sm flex flex-col">
                ...
                <div className="grid grid-cols-6 gap-2 flex-1">
                  {heatmapCells.map((val, i) => (
                    <div
                      key={i}
                      className={`aspect-square rounded-md ${val === 0 ? "bg-gray-100" : val === 1 ? "bg-[#6C5DD3]/20" : val === 2 ? "bg-[#6C5DD3]/40" : val === 3 ? "bg-[#6C5DD3]/70" : "bg-[#6C5DD3]"}`}
                    ></div>
                  ))}
                </div>
              </div>
                ...
                {alerts.map((alert) => (
                  ...
                  >
                    <div className="flex items-center gap-5">
                      <div
                        className={`${alert.bgColor} ${alert.color} p-4 rounded-2xl`}
                      >
                        <alert.icon size={24} />
                      </div>
