...
import {
  ...
  Globe,
  PlayCircle,
  ChevronDown, // Added ChevronDown here
} from "lucide-react";
            ...
            <div className="mb-10 flex justify-between items-end">
              <div>
                ...
                <p className="text-gray-400 font-bold mt-2">
                  Get technical assistance, browse system documentation, or
                  contact the Intellecta core team.
                </p>
              </div>
              <div className="relative group">
                <Search
                  className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[#6C5DD3] transition-colors"
                  size={20}
                />
                <input
                  type="text"
            ...
            <div className="grid grid-cols-12 gap-8">
              ...
              <div className="col-span-12 lg:col-span-4 bg-[#6C5DD3] p-10 rounded-[40px] text-white shadow-2xl shadow-indigo-100 h-fit">
                ...
                <div className="mt-12 pt-8 border-t border-white/10">
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-60 mb-4 text-center">
                    Contact Priority Line
                  </p>
...
const SupportCard = ({ icon, title, desc, buttonText, primary = false }) => (
  <div
    className={`p-8 rounded-[35px] border ${primary ? "border-[#6C5DD3] bg-white" : "border-gray-100 bg-white"} shadow-sm flex flex-col justify-between group hover:shadow-xl hover:-translate-y-1 transition-all duration-300`}
  >
    <div>
      <div
        className={`${primary ? "text-[#6C5DD3] bg-[#F5F6FF]" : "text-gray-400 bg-gray-50"} w-14 h-14 rounded-2xl flex items-center justify-center mb-6`}
      >
        {icon}
      </div>
    ...
    <button
      className={`mt-8 flex items-center justify-between w-full font-black text-[11px] uppercase tracking-widest ${primary ? "text-[#6C5DD3]" : "text-gray-400 group-hover:text-[#111827]"}`}
    >
      {buttonText}
...
const FAQItem = ({ question }) => (
  <div className="flex items-center justify-between p-5 hover:bg-gray-50 rounded-2xl cursor-pointer transition-colors border-b border-gray-50 group">
    ...
    <ChevronDown
      size={18}
      className="text-gray-300 group-hover:text-[#6C5DD3]"
    />
  </div>
);
